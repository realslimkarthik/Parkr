...
def get_long_lat(node_id):
    nodes_df = get_nodes()
    row = nodes_df[nodes_df['node_id'] == node_id].loc[0]
...
def route_vehicle(origin, destination, directions):
    pass


# def get_parking_spot(origin, destination, time, algorithm, radius):
def get_parking_spot(origin, destination, time, radius):
    block_list = get_nodes_within_radius(destination, radius)
    if algorithm == 'd':
